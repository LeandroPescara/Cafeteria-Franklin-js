...
function agregarAlCarrito(producto) {
  ...
  if (!memoria || memoria.length === 0) {
    ...
    localStorage.setItem("cafec", JSON.stringify(nuevaMemoria));
    actualizarNumeroCarrito();
...
function restarAlCarrito(producto) {
  ...
  const indiceProducto = memoria.findIndex((cafec) => cafec.id === producto.id);
...
function actualizarNumeroCarrito() {
  let cuenta = 0;
  const memoria = JSON.parse(localStorage.getItem("cafec"));
  if (memoria && memoria.length > 0) {
